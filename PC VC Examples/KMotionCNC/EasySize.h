...
#define EASYSIZE_MINSIZE(w,s,r)																		\
if(r->right-r->left < w.right-w.left)																				\
{																										\
	if((s == WMSZ_BOTTOMLEFT)||(s == WMSZ_LEFT)||(s == WMSZ_TOPLEFT)) r->left = r->right-(w.right-w.left);			\
	else r->right = r->left+(w.right-w.left);																			\
}																										\
if(r->bottom-r->top < w.bottom-w.top)																				\
{																										\
	if((s == WMSZ_TOP)||(s == WMSZ_TOPLEFT)||(s == WMSZ_TOPRIGHT)) r->top = r->bottom-(w.bottom-w.top);	\
	else r->bottom = r->top+(w.bottom-w.top);																			\
}																										\
...
if(o & ES_HCENTER) id##_es_r = rect.Width(); else { if(r==ES_BORDER) id##_es_r = OrigClientRect.right-rect.right; else if(r==ES_KEEPSIZE) id##_es_r = rect.Width(); else {\
	GetDlgItem(r)->GetWindowRect(rect2); ScreenToClient(rect2);\
	id##_es_r = rect2.left-rect.right;}}\
if(o & ES_VCENTER) id##_es_b = rect.Height(); else  { if(b==ES_BORDER) id##_es_b = OrigClientRect.bottom-rect.bottom; else if(b==ES_KEEPSIZE) id##_es_b = rect.Height(); else {\
