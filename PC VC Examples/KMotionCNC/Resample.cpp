...
HBITMAP CResample::CreateResampledBitmap(HDC hdc,
																					 HBITMAP hBmpSource,
...
HBITMAP CResample::CreateUserFilterResampledBitmap(HDC hdc,
																										 HBITMAP hBmpSource,
																										 DWORD dwWidth, DWORD dwHeight,
...
*/
BOOL CResample::_setResampleFilter(DWORD dwFilter, PFN_FILTER * ppFnFilter, double * pdRadius)
{
	BOOL fResult;
...
HBITMAP CResample::_createResampledBitmap(HDC hdc, HBITMAP hBmpSource, DWORD dwWidth, DWORD dwHeight, PFN_FILTER pFnFilter, double dRadius)
{
	BOOL fResult;
	...
	pbSource = (BYTE *)GlobalAlloc(0, binfoSource.bmiHeader.biSizeImage);
	if ( ! pbSource)
	{
	...
	pbTarget = (BYTE *)GlobalAlloc(0, binfoTarget.bmiHeader.biSizeImage);
	if (! pbTarget )
	{
		SetLastError(E_MEMORY_ERROR);
		goto Cleanup;
...
/* _resample
  This function does the real resampling stuff.
	...
	TRUE on success
*/
BOOL CResample::_resample(BYTE * ibuf, LONG iw, LONG ih, BYTE * obuf, LONG ow, LONG oh, PFN_FILTER pFnFilter, double dRadius)
{	
	BOOL fSuccess = FALSE;
  ...
  /* Alias (pointer to DWORD ) for obuf */
	...
	h_pixel  = (LONG *) GlobalAlloc( 0, ow * MAX_CONTRIBS * sizeof(int)); /* the contributing pixels */
	h_count  = (LONG *) GlobalAlloc( 0, ow * sizeof(int)); /* how may contributions for the target pixel */
	...
	v_pixel  = (LONG *) GlobalAlloc( 0, oh * MAX_CONTRIBS * sizeof(int)); /* The contributing pixels */
	v_count  = (LONG *) GlobalAlloc( 0, oh * sizeof(int)); /* How may contributions for the target pixel */
...
/* _fillBITMAPINFO helper function
  fills a BITMAPINFO struct given a HBITMAP
	...
	TRUE on success
*/
BOOL CResample::_fillBITMAPINFO(HBITMAP hBmp, BITMAPINFO * pBinfo)
{
	BITMAP bmp;
