...
//
// to avoid chatter due to Resonance add some Sinusoidal Spindle Speed Variation (SSV)
#define SSV_RPM 100.0 // RPM
#define SSV_PERIOD 1.0 // seconds
...
void ServiceCSS(void)
{
	...
	if (*css_mode == 2 && T > css_T) // check if we are in CSS mode and it is time to update
	{
		...
		// Add in SSV
		rpm += SSV_RPM * sin(TWO_PI / SSV_PERIOD * T);
