...
int MessageBoxHandler(const wchar_t *title, const wchar_t *msg, uint32_t options) {
  std::string titlestr = kmx::wstrtostr(title);
  std::string msgstr = kmx::wstrtostr(msg);
  return kmxCtrl->OnMessageBoxCallback(titlestr.c_str(), msgstr.c_str(), options);
}
