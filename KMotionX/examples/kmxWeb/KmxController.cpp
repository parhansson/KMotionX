...
int KmxController::Initialize(){
  ...
  if(km->CheckKMotionVersion(&type,false, false)){
...
void KmxController::EmergencyStop()
{
  if(km->WaitToken(false, 1000) == KMOTION_LOCKED){
    ...
    {
      snprintf(s, MAX_LINE, "Kill %d", i+1);
    ...
    for (i=0;i<N_CHANNELS_KFLOP;i++)             // disable all servo channels
    {
...
void KmxController::Jog(int axis, int speed){
  char command[32];
  snprintf(command, 32, "Jog%d=%d", axis, speed);
