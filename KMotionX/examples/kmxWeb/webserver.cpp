...
struct mg_server * server;    // Set by start_mongoose()
...
static void show_usage_and_exit(void) {
  ...
  for (i = 0; names[i] != NULL; i += 2) {
    fprintf(stderr, "  -%s %s\n", names[i],
        names[i + 1] == NULL ? "<empty>" : names[i + 1]);
  }
  exit(EXIT_FAILURE);
}
...
static void process_command_line_arguments(char *argv[], char **options) {
  ...
  if (argv[1] != NULL && argv[1][0] != '-') {
    ...
    snprintf(cpath, sizeof(cpath), "%.*s%c%s", (int) (p - argv[0]), argv[0],
        DIRSEP, CONFIG_FILE);
  }
...
static char *get_option(char **options, const char *option_name) {
  ...
  for (i = 0; options[i] != NULL; i++)
    if (!strcmp(options[i], option_name)) return options[i + 1];
...
static void verify_existence(char **options, const char *name, int is_dir) {
  ...
  if (!path_exists(path, is_dir)) {
    notify("Invalid path for %s: [%s]: (%s). Make sure that path is either "
        "absolute, or it is relative to mongoose executable.", name, path,
        strerror(errno));
  }
}
...
static void start_mongoose(int argc, char *argv[]) {
  ...
  if (argc > 1 && !strcmp(argv[1], "-A")) {
    ...
    exit(
        modify_passwords_file(argv[2], argv[3], argv[4], argv[5]) ?
...
static void set_options(char *argv[]) {
  ...
  for (i = 0; options[i] != NULL; i += 2) {
    ...
    if (msg != NULL) {
      notify("Failed to set option [%s] to [%s]: %s", options[i],
          options[i + 1], msg);
