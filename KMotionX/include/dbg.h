...
#define log_errno(M, ...) fprintf(kmx_stderr, "[ERROR] (%s:%d: errno: %s) " M "\n", __FILE__, __LINE__, clean_errno(), ##__VA_ARGS__)

#define log_err(M, ...) if(1){__local__time__; fprintf(kmx_stderr, "[ERROR] [%d] [%s] (%s:%d) " M "\n", (int)getpid(), __time_str__, __FILE__, __LINE__, ##__VA_ARGS__);}

#define log_warn(M, ...) if(1){__local__time__; fprintf(kmx_stderr, "[WARN] [%d] [%s] (%s:%d) " M "\n", (int)getpid(), __time_str__, __FILE__, __LINE__, ##__VA_ARGS__);}
