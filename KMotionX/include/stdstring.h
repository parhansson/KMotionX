...
//
//      From this template, we intstantiate two classes:  CString and
...
//Original code within compiles on mac but not on linux
...
//      }
    ...
    //      void _cdecl Formst(CString& PCSTR szFormat, ...)
    //      void _cdecl Format(PCSTR szFormat);
    //
    // DESCRIPTION:
    //      This function does sprintf/wsprintf style formatting on CString
                ...
                const A16& v16, const A17& v17, const A18& v18)
    {
        Fmt(szFmt, FmtArg<A1>(v1)(), FmtArg<A2>(v2)(),
            ...
            FmtArg<A9>(v9)(), FmtArg<A10>(v10)(),FmtArg<A11>(v11)(),
            FmtArg<A12>(v12)(),FmtArg<A13>(v13)(),FmtArg<A14>(v14)(),
            FmtArg<A15>(v15)(),FmtArg<A16>(v16)(),FmtArg<A17>(v17)(),FmtArg<A18>(v18)());
    }
    template<class A1, class A2, class A3, class A4, class A5, class A6,
        class A7, class A8, class A9, class A10, class A11, class A12,
        class A13, class A14, class A15, class A16, class A17, class A18, class A19>
    void Format(const CT* szFmt, const A1& v1, const A2& v2, const A3& v3,
                const A4& v4, const A5& v5, const A6& v6, const A7& v7,
                const A8& v8, const A9& v9, const A10& v10, const A11& v11,
                const A12& v12, const A13& v13, const A14& v14, const A15& v15,
...
//
// If you are using MS Visual C++ and you want to export CString and
...
typedef CStdStr<char>     CString;    // a better std::string
...
{
    CString sRet(SSREF(s1));
    sRet.append(s2);
    return sRet;
}
inline CString operator+(const CString& s1, CString::value_type t)
{
    CString sRet(SSREF(s1));
    sRet.append(1, t);
    return sRet;
}
inline CString operator+(const CString& s1, PCSTR pA)
{
    CString sRet(SSREF(s1));
    sRet.append(pA);
    return sRet;
}
inline CString operator+(PCSTR pA, const CString& sA)
{
    CString sRet;
    CString::size_type nObjSize = sA.size();
    CString::size_type nLitSize =
        static_cast<CString::size_type>(sslen(pA));
...
inline CString operator+(const CString& s1, const CStdStringW& s2)
{
...
inline CString operator+(const CString& s1, PCWSTR pW)
{
    return s1 + CString(pW);
}

#ifdef UNICODE
    inline CStdStringW operator+(PCWSTR pW, const CString& sA)
    {
...
#else
    inline CString operator+(PCWSTR pW, const CString& sA)
    {
        return CString(pW) + sA;
    }
    inline CString operator+(PCSTR pA, const CStdStringW& sW)
    {
        return pA + CString(sW);
    }
#endif
...
inline CStdStringW operator+(const CStdStringW& s1, const CString& s2)
{
...
template<>
struct FmtArg<CString>
{
    explicit FmtArg(const CString& arg) : a_(arg) {}
    PCSTR operator()() const { return a_.c_str(); }
    const CString& a_;
private:
    FmtArg<CString>& operator=(const FmtArg<CString>&) { return *this; }
};
template<>
...
// we use the exact same format.
...
#ifdef _MFC_VER
    inline CArchive& AFXAPI operator<<(CArchive& ar, const CString& strA)
    {
    ...
    inline CArchive& AFXAPI operator>>(CArchive& ar, CString& strA)
    {
        CString strTemp;
        ar >> strTemp;
        strA = strTemp;
...
//      CString WUFormat(PCSTR szFormat, ...);
//
// REMARKS:
//      This function allows the caller for format and return a CString
//      object with a single line of code.
// -----------------------------------------------------------------------------

inline CString WUFormatA(PCSTR szFormat, ...)
{
    va_list argList;
    va_start(argList, szFormat);
    CString strOut;
...
inline CStdStringW WUFormatW(PCWSTR szwFormat, ...)
{
...
#ifdef SS_ANSI
#else
    inline CString WUFormatA(UINT nId, ...)
    {
        va_list argList;
        va_start(argList, nId);
    ...
    //   CString WUSysMessageA(DWORD dwError, DWORD dwLangId=SS_DEFLANGID);
    ...
    inline CString WUSysMessageA(DWORD dwError, DWORD dwLangId=SS_DEFLANGID)
    {
        CHAR szBuf[512];
...
struct StdStringLessNoCaseA
    : std::binary_function<CString, CString, bool>
{
    inline
    bool operator()(const CString& sLeft, const CString& sRight) const
    { return ssicmp(sLeft.c_str(), sRight.c_str()) < 0; }
};
...
//namespace std
//{
//  inline void swap(CString& s1, CString& s2) throw()
//  {
//      s1.swap(s2);
//  }
//  template<>
