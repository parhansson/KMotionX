...
static int WidthStyledText(Surface *surface, ViewStyle &vs, int styleOffset,
	...
	while (start < len) {
		...
		while ((endSegment+1 < len) && (static_cast<size_t>(styles[endSegment+1]) == style))
			endSegment++;
		width += surface->WidthText(vs.styles[style+styleOffset].font, text + start, (int)(endSegment - start + 1));
		start = endSegment + 1;
	}
	return width;
}
...
static int WidestLineWidth(Surface *surface, ViewStyle &vs, int styleOffset, const StyledText &st) {
	...
	while (start < st.length) {
		...
		if (st.multipleStyles) {
			...
			widthSubLine = surface->WidthText(vs.styles[styleOffset + st.style].font, st.text + start, (int)lenLine);
		}
...
void DrawStyledText(Surface *surface, ViewStyle &vs, int styleOffset, PRectangle rcText, int ascent,
	...
	if (st.multipleStyles) {
		...
		while (i < length) {
			...
			style += styleOffset;
			int width = surface->WidthText(vs.styles[style].font, st.text + start + i, (int)(end - i + 1));
			PRectangle rcSegment = rcText;
			rcSegment.left = x;
			rcSegment.right = x + width + 1;
			surface->DrawTextNoClip(rcSegment, vs.styles[style].font,
					ascent, st.text + start + i, (int)(end - i + 1),
		...
		int style = (int)(st.style + styleOffset);
		surface->DrawTextNoClip(rcText, vs.styles[style].font,
				rcText.top + vs.maxAscent, st.text + start, (int)length,
				vs.styles[style].fore.allocated,
				vs.styles[style].back.allocated);
	}
}
...
void Editor::CopySelectionRange(SelectionText *ss, bool allowLineCopy) {
	if (sel.Empty()) {
		if (allowLineCopy) {
			...
			ss->Set(textWithEndl, (int)strlen(textWithEndl) + 1,
		...
		int size = (int)(sel.Length() + delimiterLength * sel.Count());
...
void Editor::AddStyledText(char *buffer, int appendLength) {
	...
	delete []text;
	SetEmptySelection((int)(sel.MainCaret() + textLength));
}
