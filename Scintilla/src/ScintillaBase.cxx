	...
	if (ac.chooseSingle && (listType == 0)) {
		if (list && !strchr(list, ac.GetSeparator())) {
			...
			if (ac.ignoreCase) {
				...
				pdoc->InsertString(sel.MainCaret(), list, (int)lenInsert);
				SetEmptySelection(sel.MainCaret() + (int)lenInsert);
			} else {
				SetEmptySelection(sel.MainCaret());
				pdoc->InsertString(sel.MainCaret(), list + lenEntered, (int)lenInsert - lenEntered);
				SetEmptySelection(sel.MainCaret() + (int)lenInsert - lenEntered);
			}
			return;
		}
	}
